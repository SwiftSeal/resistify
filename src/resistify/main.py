...
DEFAULT_DEVICE = str(torch.device("mps" if torch.backends.mps.is_built() else "cuda" if torch.cuda.is_available() else "cpu"))
...
def main(
    ...
    device: Annotated[str, typer.Option(help="Torch device to be used - can be 'cpu', 'cuda', or 'mps'.")] = DEFAULT_DEVICE,
    quick: Annotated[bool, typer.Option(help="Enable to skip all ESM predictions.")] = False,
    batch_size: Annotated[int, typer.Option(help="Batch size for ESM predictions - reduce this value for lower memory usage.")] = 8,
    pfam: Annotated[Path, typer.Option(help="Path to the Pfam database file - by default a smaller NLR-specific database is used.")] = Path(os.path.dirname(__file__)) / "data" / "db.hmm"
):
