...
def predict_lrr(
    ...
    with Progress(console=console) as progress:
        ...
        for i in range(0, len(proteins), batch_size):
            ...
            for protein, prediction_tensor, attention_mask_tensor in zip(
                batch_proteins, predictions, inputs["attention_mask"]
            ):
                labels = prediction_tensor.tolist()
                attention_mask = attention_mask_tensor.tolist()
                # Remove padding tokens
                labels = [
                    label for label, mask in zip(labels, attention_mask) if mask == 1
                ]
                ...
                if motif_positions:
                    ...
                    for pos in motif_positions:
                        ...
                        protein.add_annotation(
                            Annotation("LxxLxL", pos + 1, pos + 6, type="motif")
                        )
                        if pos - end < lrr_gap:
